  ...
  useEffect(() => {
    ...
    if (slots && slots.length > 0) {
      slots.forEach((slot) => {
        ...
        // Draw slot rectangle
        ctx.lineWidth = 2;
        ctx.strokeStyle = slot.is_occupied ? '#e74c3c' : '#27ae60';
        ctx.fillStyle = slot.is_occupied ? 'rgba(231, 76, 60, 0.2)' : 'rgba(39, 174, 96, 0.2)';
        ctx.fillRect(x, y, w, h);
        ctx.strokeRect(x, y, w, h);

        // Draw slot label
        ctx.fillStyle = slot.is_occupied ? '#e74c3c' : '#27ae60';
    ...
    ctx.fillText('Parking Layout', 10, 10);
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText('Occupied', 170, legendY + 2);
