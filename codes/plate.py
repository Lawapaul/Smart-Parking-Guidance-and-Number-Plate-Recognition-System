...
PLATE_MODEL_PATH = PROJECT_DIR / "license_plate_detector.pt"
...
def locate_plate_in_vehicle(
    ...
    plate_result = plate_model(search_roi, verbose=False)[0]
    if len(plate_result.boxes) > 0:
        best_box = max(
            plate_result.boxes,
            key=lambda box: float(box.conf[0].item()) * max(
                1.0,
                (box.xyxy[0][2] - box.xyxy[0][0]).item() * (box.xyxy[0][3] - box.xyxy[0][1]).item()
            ),
        )
        px1, py1, px2, py2 = map(int, best_box.xyxy[0].tolist())
        return (
            x1 + search_x1 + px1,
            y1 + search_y1 + py1,
            x1 + search_x1 + px2,
...
def main() -> int:
    ...
    if not plate_model_path.exists():
        print(f"Missing plate detection model: {plate_model_path}")
        return 1
    ...
    while True:
        ...
        if stable_vehicle is not None:
            plate_box = locate_plate_in_vehicle(frame, stable_vehicle, plate_model)
            if plate_box is not None:
